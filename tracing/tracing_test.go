...
func TestInitWithTLSValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid config with both metrics enabled",
			config: Config{
				HostMetricsEnabled:     true,
				RuntimeMetricsEnabled:  true,
				HostMetricsInterval:    5 * time.Second,
...
func TestInitBackwardCompatibility(t *testing.T) {
	...
	// Initialize without TLS config for backward compatibility
	_, provider, err := Init(ctx, config)
	if err != nil {
...
func TestInitWithErrorScenarios(t *testing.T) {
	// Save original env vars and restore them after test
	originalVars := map[string]string{
		"OTEL_SERVICE_NAME":                  os.Getenv("OTEL_SERVICE_NAME"),
		"OTEL_EXPORTER_OTLP_TRACES_PROTOCOL": os.Getenv("OTEL_EXPORTER_OTLP_TRACES_PROTOCOL"),
...
func TestInitConcurrency(t *testing.T) {
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(i int) {
			...
			_, provider, err := Init(ctx, config)
			if err != nil {
				t.Errorf("Tracing Init failed for goroutine %d: %v", i, err)
				return
			}
			if provider != nil {
				_ = Shutdown(ctx, provider)
...
func TestInitErrorPaths(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid config with host metrics enabled",
			config: Config{
				HostMetricsEnabled: true,
			},
			wantErr: false,
		...
		{
			name: "valid config with runtime metrics enabled",
			config: Config{
				RuntimeMetricsEnabled: true,
			},
			wantErr: false,
...
func TestContextCancellation(t *testing.T) {
	...
	// Shutdown tracing provider with the same context
	if err := Shutdown(ctx, provider); err != nil {
...
func TestConfigurationValidation(t *testing.T) {
	...
	// Test with custom config to ensure defaults are properly preserved
	configWithMetrics := Config{
		HostMetricsEnabled: true,
	}
	_, provider2, err := Init(ctx, configWithMetrics)
	if err != nil {
		t.Errorf("Init with custom config should succeed: %v", err)
	}
	if provider2 == nil {
		t.Error("Provider should not be nil with custom config")
	}
